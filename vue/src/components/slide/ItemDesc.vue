<script setup lang="ts">
import { computed, inject, reactive } from 'vue'
import { _formatNumber } from '@/utils'

const props = defineProps({
  isMy: {
    type: Boolean,
    default: () => {
      return false
    }
  },
  isLive: {
    type: Boolean,
    default: () => {
      return false
    }
  }
})

const item = inject<any>('item')
const currentItem = computed(() => item?.value ?? item ?? {})
const liveLocation = computed(() => {
  return currentItem.value?.city || currentItem.value?.address || currentItem.value?.room_no || ''
})
const liveAuthorName = computed(() => {
  return currentItem.value?.author?.nickname || currentItem.value?.persona?.name || '官方直播间'
})
const liveTitle = computed(() => {
  return (
    currentItem.value?.title ||
    currentItem.value?.desc ||
    currentItem.value?.author?.signature ||
    currentItem.value?.persona?.name ||
    '精彩直播进行中'
  )
})
const liveSubtitle = computed(() => {
  return currentItem.value?.subtitle || currentItem.value?.music?.title || ''
})
const liveTags = computed(() => {
  const tags = currentItem.value?.persona?.tags ?? []
  return tags.filter(Boolean).slice(0, 3)
})
const liveOnlineText = computed(() => {
  return normalizeCountText(
    currentItem.value?.display?.online_text ?? currentItem.value?.statistics?.comment_count,
    '在线互动'
  )
})
const liveLikeText = computed(() => {
  return normalizeCountText(
    currentItem.value?.display?.like_text ?? currentItem.value?.statistics?.digg_count,
    '热度提升'
  )
})
const liveGiftText = computed(() => {
  const gifts = currentItem.value?.gift_panel?.quick_gifts
    ?.map((gift) => gift?.name)
    .filter(Boolean)
    .slice(0, 2)
  if (gifts?.length) {
    return gifts.join(' / ')
  }
  return currentItem.value?.state?.privilege_active ? '特权礼物已开启' : '支持弹幕刷礼物'
})

function normalizeCountText(value: string | number | undefined, fallback: string) {
  if (value === undefined || value === null || value === '') return fallback
  if (typeof value === 'string') return value
  return _formatNumber(value) ?? String(value)
}

const state = reactive({
  isAttention: false,
  test: [1, 2]
})
</script>
<template>
  <div class="item-desc ml1r mb1r" :class="{ 'is-live': props.isLive }">
    <div class="content" v-if="!props.isMy">
      <div class="location-wrapper" v-if="!props.isLive && (currentItem.city || currentItem.address)">
        <div class="location">
          <img src="../../assets/img/icon/location.webp" alt="" />
          <span>{{ currentItem.city }}</span>
          <template v-if="currentItem.address">
            <div class="gang"></div>
          </template>
          <span>{{ currentItem.address }}</span>
        </div>
      </div>
      <template v-if="props.isLive">
        <div class="location-wrapper" v-if="liveLocation">
          <div class="location">
            <img src="../../assets/img/icon/location.webp" alt="" />
            <span>{{ liveLocation }}</span>
          </div>
        </div>
        <div class="live-meta">
          <div class="live">直播中</div>
          <div class="stat">{{ liveOnlineText }}在线</div>
          <div class="stat">{{ liveLikeText }}点赞</div>
        </div>
      </template>
      <div class="name mb1r f18 fb" @click.stop="$emit('goUserInfo')">
        @{{ liveAuthorName }}
      </div>
      <div v-if="props.isLive" class="live-title">
        {{ liveTitle }}
      </div>
      <div class="description">
        {{ props.isLive ? liveSubtitle || currentItem.desc : currentItem.desc }}
      </div>
      <div v-if="props.isLive && liveTags.length" class="live-tags">
        <span class="tag" :key="tag" v-for="tag in liveTags">{{ tag }}</span>
      </div>
      <div v-if="props.isLive" class="live-gift">
        快捷礼物：{{ liveGiftText }}
      </div>
      <!--      <div class="music" @click.stop="bus.emit('nav','/home/music')">-->
      <!--        <img src="../../assets/img/icon/music.svg" alt="" class="music-image">-->
      <!--        <span>{{ item.music.title }}</span>-->
      <!--      </div>-->
    </div>
    <div v-else class="comment-status">
      <div class="comment">
        <div class="type-comment">
          <img src="../../assets/img/icon/head-image.jpeg" alt="" class="avatar" />
          <div class="right">
            <p>
              <span class="name">zzzzz</span>
              <span class="time">2020-01-20</span>
            </p>
            <p class="text">北京</p>
          </div>
        </div>
        <transition-group name="comment-status" tag="div" class="loveds">
          <div class="type-loved" :key="i" v-for="i in state.test">
            <img src="../../assets/img/icon/head-image.jpeg" alt="" class="avatar" />
            <img src="../../assets/img/icon/love.svg" alt="" class="loved" />
          </div>
        </transition-group>
      </div>
    </div>
  </div>
</template>

<style scoped lang="less">
.item-desc {
  position: absolute;
  bottom: 0;
  width: 70%;

  &.is-live {
    pointer-events: none;
    z-index: 1;

    .name {
      pointer-events: auto;
    }
  }

  .content {
    color: #fff;
    text-align: left;

    .location-wrapper {
      display: flex;

      .location {
        margin-bottom: 10rem;

        display: flex;
        align-items: center;
        font-size: 12rem;
        padding: 4rem;
        border-radius: 3rem;
        background: var(--second-btn-color-tran);

        .gang {
          height: 8rem;
          width: 1.5px;
          margin: 0 5rem;
          background: gray;
        }

        img {
          margin-right: 7rem;
          width: 18rem;
        }
      }
    }

    .live {
      border-radius: 3rem;
      padding: 3rem 6rem;
      font-size: 11rem;
      display: inline-flex;
      background: var(--primary-btn-color);
      color: white;
    }

    .live-meta {
      display: flex;
      align-items: center;
      flex-wrap: wrap;
      gap: 8rem;
      margin-bottom: 10rem;

      .stat {
        padding: 3rem 8rem;
        border-radius: 999rem;
        font-size: 11rem;
        background: rgba(255, 255, 255, 0.12);
      }
    }

    .live-title {
      margin-bottom: 8rem;
      font-size: 17rem;
      font-weight: 700;
      line-height: 1.4;
    }

    .live-tags {
      display: flex;
      flex-wrap: wrap;
      gap: 6rem;
      margin-top: 10rem;

      .tag {
        padding: 4rem 8rem;
        border-radius: 999rem;
        font-size: 11rem;
        background: rgba(255, 255, 255, 0.12);
      }
    }

    .live-gift {
      margin-top: 10rem;
      font-size: 11rem;
      color: rgba(255, 255, 255, 0.82);
    }

    .music {
      position: relative;
      display: flex;
      align-items: center;

      .music-image {
        width: 20px;
        height: 20px;
      }
    }
  }

  .comment-status {
    display: flex;
    align-items: center;

    .comment {
      .type-comment {
        display: flex;
        background: rgb(130, 21, 44);
        border-radius: 50px;
        padding: 3px;
        margin-bottom: 20px;

        .avatar {
          width: 36px;
          height: 36px;
          border-radius: 50%;
        }

        .right {
          margin: 0 10px;
          color: var(--second-text-color);

          .name {
            margin-right: 10px;
          }

          .text {
            color: white;
          }
        }
      }

      .loveds {
      }

      .type-loved {
        width: 40px;
        height: 40px;
        position: relative;
        margin-bottom: 20px;
        animation: test 1s;
        animation-delay: 0.5s;

        .avatar {
          width: 36px;
          height: 36px;
          border-radius: 50%;
        }

        .loved {
          position: absolute;
          bottom: 0;
          left: 20px;
          width: 10px;
          height: 10px;
          background: red;
          padding: 3px;
          border-radius: 50%;
          border: 2px solid white;
        }
      }

      @keyframes test {
        from {
          display: block;
          transform: translate3d(0, 0, 0);
        }
        to {
          display: none;
          transform: translate3d(0, -60px, 0);
        }
      }
    }
  }
}
</style>
